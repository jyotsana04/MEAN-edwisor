...
eventEmitter.on('welcomeEmail', function(userdetails){
    ...
    var mailOptions = {
        ...
        text : `Hello ${fullName}, Thank you for signing up to my app.`
    }
    ...
    let sendTokenMail = (result)=>{
        return new Promise((resolve, reject)=>{
            ...
            var mailOptions = {
                ...
                html: '<p>Click <a href="http://localhost:4200/reset/${result.resetPasswordToken}"> here</a>`
            }
            ...
            smtpTransport.sendMail(mailOptions, function(error, info){
                ...
                if(error){
                    console.log(error)
                    let apiResponse = response.generate(true, 'mail cannot be sent.', 500, null)
                    reject(apiResponse)
                }else {
                    console.log('Email sent' + info.response)
                    let apiResponse = response.generate(false, 'mail sent  Successfully.', 200, info.message)
                    resolve(apiResponse)
                }
            })
...
//starting of setting new password
// first version
/*
let setNewPassword = (req, res)=>{

    UserModel.findOne({resetPasswordToken: req.body.token, resetPasswordExpires : {$gt : Date.now()}})
        .exec((err, userdetails)=>{
            if(err){
                console.log('error while fetching user details '+ err)
                let apiResponse = response.generate(true, 'invalid or wrong token', 500, null)
                res.send(apiResponse)
            }else {
